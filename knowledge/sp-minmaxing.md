...
4. **Optimize:** run [tools/sp-optimizer](../tools/sp-optimizer/README.md) — `python3 optimize.py <skills.csv> <budget>` — an exact 0/1 knapsack with prerequisite chains. Greedy by L-per-SP is a good sanity check but can be beaten by the exact solve.
